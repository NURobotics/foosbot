...
BALL_COLOR_IMG = "test/ball_color_webcam.png"
FPS = 30
...
def color_match(frame):
  global ball_color_rgb
  if ball_color_rgb is None:
    ball_color_rgb = get_ball_color()
    print(ball_color_rgb)
  frame = cv.medianBlur(frame,5)
  frame_with_ball_color = np.tile(ball_color_rgb, (frame.shape[0], frame.shape[1], 1))
  frame = np.abs(np.subtract(frame, frame_with_ball_color))
  frame = cv.cvtColor(frame.astype("uint8"), cv.COLOR_BGR2GRAY)
  black_white_frame = np.zeros_like(frame)
  black_white_frame[frame < 15] = 255
  black_white_frame[frame > 240] = 255
...
#     return _frame_circle[img_path]
  ...
  circles_found = cv.HoughCircles(img, cv.HOUGH_GRADIENT, 2, 20, param1=30, param2=5, minRadius=50, maxRadius=150)
...
def draw_circles(frame):
  circle = find_circle(color_match(frame), img_path="test")
  if circle is None:
    # print("could not draw circles because there is no circle")
    return frame
  return cv.circle(frame, (int(circle[0]), int(circle[1])), int(circle[2]), (20,255,57), thickness=5)
